...
playground:
	...
	@echo "|                                                                             |"
	@echo "| 🔍 IMPORTANT: Select the 'agent08_github_a2a_agent_mcp_server' folder to interact with your agent.          |"
	@echo "==============================================================================="
...
deploy:
	...
	(uv export --no-hashes --no-header --no-dev --no-emit-project --no-annotate > agent08_github_a2a_agent_mcp_server/app_utils/.requirements.txt 2>/dev/null || \
	uv export --no-hashes --no-header --no-dev --no-emit-project > agent08_github_a2a_agent_mcp_server/app_utils/.requirements.txt) && \
	uv run -m agent08_github_a2a_agent_mcp_server.app_utils.deploy \
		--source-packages=./agent08_github_a2a_agent_mcp_server \
		--entrypoint-module=agent08_github_a2a_agent_mcp_server.agent_engine_app \
		--entrypoint-object=agent_engine \
		--requirements-file=agent08_github_a2a_agent_mcp_server/app_utils/.requirements.txt \
...
eval:
	...
	uv run adk eval ./agent08_github_a2a_agent_mcp_server $${EVALSET:-tests/eval/evalsets/basic.evalset.json} \
